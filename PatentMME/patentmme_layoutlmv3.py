...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):
    def __init__(self, config, calc_pch_loss, calc_tna_loss, train_head_only):
        super().__init__(config, calc_pch_loss, calc_tna_loss, train_head_only)
        self.config = config
        self.calc_pch_loss = calc_pch_loss
        self.calc_tna_loss = calc_tna_loss
        
        self.patch_shape = (config.input_size // config.patch_size, config.input_size // config.patch_size)
        ...
        if config.visual_embed:
            ...
            self.pos_embed = nn.Parameter(torch.zeros(1, 14 * 14 + 1, config.hidden_size))
            self.pos_drop = nn.Dropout(p=0.0)

            #init VQ-GAN
            vqgan_config = OmegaConf.load(config.vqgan_config_path)
            vqgan_config.model.ckpt_path = config.vqgan_ckpt_path
            self.vqgan = instantiate_from_config(vqgan_config.model)
            self.vqgan.requires_grad_(False)
            
            ### MIM
            self.vis_mask_token = nn.Parameter(torch.zeros(1, 1, config.hidden_size))
            self.mim_head = ProjectorMLP(config.hidden_size, vqgan_config.model.params.n_embed + 1)
            ### /MIM

            ### MLM
            self.mlm_head = ProjectorMLP(config.hidden_size, config.vocab_size)
            ### /MLM

            ### PCH
            self.patch_cls_linear_head = ProjectorMLP(config.hidden_size, config.n_patch_classes)
            self.sigmoid = nn.Sigmoid()
            ### /PCH

            # ### TBA
            self.tba_linear_head = ProjectorMLP(config.hidden_size, config.n_nodes + 1)
            # ### /TBA

            ### TNA
            self.tna_ocr_head = ProjectorMLP(config.hidden_size, config.hidden_size//2)
            self.tna_vis_head = ProjectorMLP(config.hidden_size, config.hidden_size//2)
    ...
    def forward_image(self, pixel_values, pch_gt_anns, mask_im=True, labels=None):
        embeddings = self.patch_embed(pixel_values)

        ### MIM
        labels = None
        if mask_im:
            #TODO: get this working with VQ-GAN
            with torch.no_grad():
                _, _, (_, _, labels) = self.vqgan.encode(pixel_values)
                labels = labels.reshape(pixel_values.shape[0], -1).detach()
            embeddings, labels = self.mask_vis_tokens_for_mim(embeddings, labels, pch_gt_anns)
        ### /MIM
        ...
        if position_embedding is not None:
            # interpolate the position embedding to the corresponding size
            position_embedding = position_embedding.view(1, 14, 14, -1)
            position_embedding = position_embedding.permute(0, 3, 1, 2) #(1, 1024, 14, 14)
            patch_height, patch_width = 24, 24
            position_embedding = F.interpolate(position_embedding, size=(patch_height, patch_width), mode="bicubic")
            
            position_embedding = position_embedding.permute(0, 2, 3, 1) #(1, 24, 24, 1024)
            position_embedding = position_embedding.view(1, -1, 1024)
            embeddings = embeddings + position_embedding

        # add [CLS] token
        batch_size, seq_len, _ = embeddings.size()
        cls_tokens = self.cls_token.expand(batch_size, -1, -1) + self.pos_embed[:,0].expand(batch_size, -1, -1)
    ...
    # MLM #
    def mask_text_tokens_for_mlm(self, input_ids, attention_mask):
        # create a copy of input_ids to be used as labels for MLM
        labels = input_ids.detach().clone()

        # iterate through batch
        for seq_id in range(input_ids.shape[0]):
            indices = range(0, input_ids.shape[1])
            indices_wo_sp_tokens = torch.where(attention_mask[seq_id]!=0)[0][1:-1].tolist()

            # mask mask_proba % of the tokens in each sequence
            mask_indices = random.sample(indices_wo_sp_tokens, int(self.config.txt_mask_prob*len(indices_wo_sp_tokens)))

            for i in indices:
                if i not in mask_indices:
                    labels[seq_id,i] = -100
                else:
                    input_ids[seq_id,i] = 50264
        
        return input_ids, labels

    def calc_masked_lang_modeling_loss(self, txt_hidden, labels):
        txt_hidden = self.mlm_head(txt_hidden)
        loss = F.cross_entropy(txt_hidden.reshape(-1, txt_hidden.shape[-1]), labels.reshape(-1), reduction='none')
        loss[torch.isnan(loss)] = 0.
        return loss.mean()
    # /MLM #

    ##TNA
    def calc_text_node_alignment_loss(self, vis_embeds, txt_embeds, tna_labels, node_patch_mapping):
        vis_embeds = self.tna_vis_head(vis_embeds)
        txt_embeds = self.tna_ocr_head(txt_embeds)
        
        # create node embedding
        node_embeds = (node_patch_mapping @ vis_embeds) / (node_patch_mapping.sum(-1, keepdim=True) + 1e-6)
        ...
        sim = torch.matmul(txt_embeds, node_embeds.transpose(1, 2)).permute(0, 2, 1)

        # calculate loss
        loss = F.cross_entropy(sim, tna_labels, reduction='none')
        loss[torch.isnan(loss)] = 0.
    ...
    # MIM
    # def calc_structure_sensitive_patches(self, im_anns):
    #     n_patches_x = self.config.input_size / self.config.patch_size
    #     allowed_patches = []
    #     # iterate through bounding boxes for different categories
    #     for ctg_bbox in im_anns:
    #         x_min = ctg_bbox[0]
    #         y_min = ctg_bbox[1]
    #         x_max = ctg_bbox[2]
    #         y_max = ctg_bbox[3]
    #         # +1 for indexing starting from 1 and not 0
    #         strt_patch_id = (y_min//self.config.patch_size)*n_patches_x + (x_min//self.config.patch_size) + 1
    #         end_patch_id = (y_max//self.config.patch_size)*n_patches_x + (x_max//self.config.patch_size) + 1
    #         i = -1
    #         while True:
    #             i += 1
    ...
    #             allowed_patches.extend(range(new_row, new_row + int(x_max - x_min)//self.config.patch_size + 1 + 1))

    #     allowed_patches = list(set(allowed_patches))
    #     return allowed_patches
    

    def mask_vis_tokens_for_mim(self, x, labels, pch_gt_anns):
        mask = torch.ones(x.shape[0], x.shape[1])
        # iterate through batch for image annotations
        for batch_id, im_anns in enumerate(pch_gt_anns):
            allowed_patches = torch.nonzero(torch.sum(im_anns, -1), as_tuple=True)[0].tolist()
            # allowed_patches = self.calc_structure_sensitive_patches(im_anns)
            
            # randomly select patches to be masked [different for each image of the batch]
            mask_patch_ids = random.sample(allowed_patches, int(self.config.vis_mask_prob * len(allowed_patches)))

            for idx in mask_patch_ids:
                mask[batch_id, idx] = 0.
        mask = mask.unsqueeze(-1).to(x.device)
        x = mask * x + (1. - mask) * self.vis_mask_token

        if labels.shape[0] == 1:
            labels[mask.squeeze().unsqueeze(0) == 1] = -100
        else:
            labels[mask.squeeze() == 1] = -100

        return x, labels


    def calc_masked_image_modeling_loss(self, vis_hidden, labels):
        vis_hidden = self.mim_head(vis_hidden)
        loss = F.cross_entropy(vis_hidden.reshape(-1, vis_hidden.shape[-1]), labels.reshape(-1), reduction='none')
        loss[torch.isnan(loss)] = 0.
        return loss.mean()
    # /MIM

    # PCH
    def calc_patch_classification_loss(self, vis_hidden, pch_gt_anns):
        vis_hidden = self.patch_cls_linear_head(vis_hidden)
        vis_hidden = self.sigmoid(vis_hidden)
        
        return F.binary_cross_entropy(vis_hidden, pch_gt_anns)
    # /PCH

    # TBA
    # def calc_text_node_alignment_loss(self, txt_hidden, tba_labels):
    #     txt_hidden = self.tba_linear_head(txt_hidden)
    #     loss = F.cross_entropy(txt_hidden.reshape(-1, txt_hidden.shape[-1]), tba_labels.reshape(-1), reduction='none')
    #     loss[torch.isnan(loss)] = 0.
    #     return loss.mean()
    # /TBA
    ...
    def forward(
        ...
        pch_gt_anns: Optional[torch.LongTensor] = None,
        tna_gt_anns: Optional[torch.LongTensor] = None,
        nodeid_patch_map: Optional[torch.LongTensor] = None,
        ...
        ### MLM
        input_ids, txt_labels = self.mask_text_tokens_for_mlm(input_ids, attention_mask)
        ...
        if pixel_values is not None:
            ...
            visual_embeddings, visual_labels = self.forward_image(pixel_values, pch_gt_anns, mask_im=True)
            ...
            text_embed_dim = embedding_output.shape
        ...
        text_outputs = encoder_outputs['last_hidden_state'][:,:text_embed_dim[1],:]
        vis_outputs = encoder_outputs['last_hidden_state'][:, text_embed_dim[1]+1:, :]      # do not include [CLS] for MIM for patch cls
        ...
        ### MLM
        mlm_loss = self.calc_masked_lang_modeling_loss(text_outputs, txt_labels)
        ### /MLM

        ### MIM
        mim_loss = self.calc_masked_image_modeling_loss(vis_outputs, visual_labels)
        ### /MIM

        loss = mlm_loss + mim_loss

        pch_loss = 0.
        if self.calc_pch_loss:
            ### PCH
            pch_loss = self.calc_patch_classification_loss(vis_outputs, pch_gt_anns)
            ###/PCH
            loss += pch_loss

        tna_loss = 0.
        if self.calc_tna_loss:
            ### TNA
            if nodeid_patch_map.shape[1] != 0:
                tna_loss = self.calc_text_node_alignment_loss(vis_outputs, text_outputs, tna_gt_anns, nodeid_patch_map)
            else:
                tna_loss = (encoder_outputs['last_hidden_state']*0.).sum()
            ### /TNA
            loss += tna_loss
